...
public class CHomeController extends SelectorComposer<Component> {
    ...
    public void onClickbuttonLogout (Event event) {
        ...
        Messagebox.show("are you sure you want to logout? ", "Logout", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
            public void onEvent(Event evt) throws InterruptedException {
              
                if (evt.getName().equals("onOK")) {
                  //aca se hace el logout
                    
                    if ( controllerLogger != null )
                        controllerLogger.logMessage( "1", CLanguage.translateIf( controllerLanguage, "Logout confirm accepted" ) ); 
                    
                  Sessions.getCurrent().invalidate(); //se obliga a limpiar la lista de forma mas productiva que con remove atribute   
